...
system_msg_qa = "Answer the input question with 4-5 complete sentences. Provide detailed and informative responses. Include relevant context and examples when appropriate."
# Original short-answer version: "Always respond to the input question concisely with a short phrase or a single-word answer. Do not repeat the question or provide any explanation."
...
def get_response(hparams, model, tok, messages, max_new_tokens=1, eval_flag=False, device_eval='cuda:0'): 
    ...
    # Use chat template for all cases to maintain natural generation
    ...
    response = tok.decode(output_ids[0][msg_tokenized['input_ids'].shape[-1]:], skip_special_tokens=True)
    
    # Only strip for short responses (accuracy testing with max_new_tokens <= 20)
    if max_new_tokens <= 20:
        response = response.replace('\n', ' ').strip().rstrip('.')
    else:
        # For longer responses, only strip leading/trailing whitespace, preserve newlines
...
def test_prediction_acc(hparams, model_qa, tok_qa, model_eval, tok_eval, device_eval, prompt_qa, label, pre_or_post, vanilla_generation=False, system_msg=system_msg_qa):
    if vanilla_generation and pre_or_post=='post':
        ...
        target_new_tokens_len = 150  # Changed from label-based length to fixed 150 for detailed responses
...
def test_prediction_acc_single(hparams, model_qa, tok_qa, model_eval, tok_eval, device_eval, prompt_qa, label, system_msg_qa):
    ...
    output_qa = get_response(hparams, model_qa, tok_qa, messages_qa, max_new_tokens=150)  # Changed from 16 to 150 for detailed responses
...
def test_prediction_acc_multi_turn(hparams, model_qa, tok_qa, model_eval, tok_eval, device_eval, prompt_qa, label, multi_turn, yes_question, no_question, pre_or_post, vanilla_generation=False):
    ...
    if vanilla_generation and pre_or_post=='post':
        target_new_tokens_len = 150  # Changed from label-based to fixed 150 for detailed responses
        ...
        current_output = get_response(hparams, model_qa, tok_qa, messages_qa, max_new_tokens=150)
    # if 'gpt_j' in model_qa_name.lower():
...
class BaseEditor:
    ...
    def edit(self,
             ...
             skip_pre_edit=False,  # Skip pre-edit evaluation to save time
             skip_weight_restore=False,  # Skip weight restoration for logit lens analysis
             eval_model_id='./hugging_cache/llama-3-8b-instruct',
             device_eval='cuda:0',
             multi_turn=None,
             **kwargs
             ):
        ...
        # Reuse editing model as eval model to avoid loading twice and BFloat16 issues
        if summary_metrics:
            model_eval = self.model  # Use editing model for evaluation
            tok_eval = self.tok      # Use editing tokenizer
        else:
            model_eval = None
            tok_eval = None
        ...
        if 'pre_edit' in kwargs and kwargs['pre_edit'] is not None:
            ...
            all_metrics = metrics
        elif not summary_metrics or skip_pre_edit:
            # Skip pre-edit metrics calculation if summary_metrics is False or skip_pre_edit is True
            for i, request in enumerate(requests):
        ...
        for i, request in enumerate(requests):
            ...
            start = time()
            if summary_metrics:
                # Only compute post-edit metrics if summary_metrics is True
                if self.alg_name in ['IKE', 'ICL']:
                    all_metrics[i].update({
                        'case_id': i,
                        "requested_edit": request,
                ...
                # Skip post-edit metrics calculation
                all_metrics[i].update({
                    'case_id': i,
                    "requested_edit": request,
                    "time": exec_time,
            ...
            # Restore weights if keep_original_weight is True (unless skip_weight_restore is True)
            if not skip_weight_restore:
                if self.alg_name == 'KN' or self.alg_name == 'WISE' or (self.alg_name == 'GRACE' and keep_original_weight):
                    with torch.no_grad():
                ...
                request.update(
                    {
                        ...
                        'loc_prompt': kwargs['subject'][i]  # Add loc_prompt for WISE
                    }
                )
